...
class RedisService():
    ...
    async def handle_two_way_chat_communication(self, websocket: WebSocketResponse, channel_name: str) -> None:
        pubsub = self._create_subscribed_pubsub(channel_name=channel_name)

        try:
            await self._run_chat_communication(websocket=websocket, pubsub=pubsub, channel_name=channel_name)
    ...
    async def _run_chat_communication(self, websocket: WebSocketResponse, pubsub: PubSub, channel_name: str) -> None:
        tasks: list[asyncio.Task] = [
            asyncio.create_task(self.receive_chat_message(websocket=websocket, pubsub=pubsub)),
            asyncio.create_task(self.publish_chat_message(websocket=websocket, channel_name=channel_name))
        ]
